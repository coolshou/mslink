...
Usage :
 ${0} -l target_of_file_lnk [-n description] [-w working_dir] [-a cmd_args] [-i icon_path] -o my_filename.lnk [-p]
...
# We convert the values of the targets into binary
TARGET_ROOT=$(ascii2hex "${TARGET_ROOT}")
TARGET_ROOT=${TARGET_ROOT}$(for i in `seq 1 21`;do echo -n '\x00';done) # Necessary from Vista and higher, otherwise the link is considered empty (I haven't found any information on this subject)
...
echo "Creation of a shortcut of type \""${TYPE_TARGET}" "${TYPE_LNK}"\" with the target "${LNK_TARGET} ${param_HasArguments} 1>&2
